...
from AaruMusic import YouTube, app
from AaruMusic.core.call import Aaru
from AaruMusic.misc import SUDOERS, db
from AaruMusic.utils.database import (
    get_active_chats,
    get_lang,
    get_upvote_count,
    is_active_chat,
...
from AaruMusic.utils.decorators.language import languageCB
from AaruMusic.utils.formatters import seconds_to_min
from AaruMusic.utils.inline import close_markup, stream_markup, stream_markup_timer
from AaruMusic.utils.stream.autoclear import auto_clean
from AaruMusic.utils.thumbnails import gen_thumb
from config import (
    BANNED_USERS,
    SOUNCLOUD_IMG_URL,
    STREAM_IMG_URL,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Aaru.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Aaru.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Aaru.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await gen_thumb(videoid)
            ...
            try:
                await Aaru.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
