...
import config
from AaruMusic import LOGGER, app, userbot
from AaruMusic.core.call import Aaru
from AaruMusic.misc import sudo
from AaruMusic.plugins import ALL_MODULES
from AaruMusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("AaruMusic.plugins" + all_module)
    LOGGER("AaruMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Aaru.start()
    try:
        await Aaru.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("AaruMusic").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("AaruMusic").info("Stopping Aaru Music Bot...")
